/*
  # Fix Timestamp Type Mismatch Migration

  1. Purpose
     - Fix timestamp type mismatches in database functions
     - Ensure all timestamp columns use timestamptz consistently
     - Update functions to return correct timestamp types

  2. Changes
     - Drop functions and triggers in correct order with CASCADE
     - Update users table timestamp columns to timestamptz
     - Recreate functions with proper timestamp types
...
-- Drop trigger first, then function (proper dependency order)
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS handle_new_user() CASCADE;

-- Drop other functions that need to be updated
DROP FUNCTION IF EXISTS get_or_create_user_profile(uuid) CASCADE;
DROP FUNCTION IF EXISTS get_user_profile(uuid) CASCADE;

-- Ensure the users table timestamp columns are properly typed
DO $$
BEGIN
...
-- Recreate handle_new_user function with proper timestamp handling
...
-- Recreate get_or_create_user_profile function with correct timestamp types
...
BEGIN
    ...
    IF FOUND THEN
      BEGIN
        INSERT INTO users (
          ...
          END,
          true,
          COALESCE(auth_user_data.created_at, now())  -- Use COALESCE with now() as fallback
        );
  ...
  SELECT 
    ...
    u.created_at,  -- Already timestamptz after column update
    u.last_login   -- Already timestamptz after column update
  FROM users u
  WHERE u.id = user_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Recreate get_user_profile function with correct timestamp types
