/*
  # Fix User Profile Creation with Password Hash Handling

  1. Database Changes
    - Make password_hash column nullable for auth users
    - Create enhanced trigger for user profile creation
    - Add functions for missing profile recovery
...
-- First, make password_hash nullable since auth users don't need it
...
BEGIN
  ...
  -- Insert user profile with all necessary fields (password_hash is NULL for auth users)
...
BEGIN
  ...
  LOOP
    ...
    EXCEPTION
      WHEN OTHERS THEN
        RAISE LOG 'Failed to create user profile for %: % - %', auth_user.id, SQLSTATE, SQLERRM;
    END;
  END LOOP;
...
BEGIN
  ...
  IF user_count = 1 THEN
    ...
    RAISE LOG 'Promoted first user to admin: %', NEW.id;
  END IF;
...
DROP TRIGGER IF EXISTS promote_first_user_to_admin_trigger ON public.users;
