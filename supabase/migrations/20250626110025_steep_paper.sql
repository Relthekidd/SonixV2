/*
  # User Profile System with Social Features

  1. New Tables
    - `user_follows` - Follow relationships between users
    - `follower_requests` - Follow requests for private accounts
    - `user_top_artists` - User's top listened artists tracking
    - `user_top_songs` - User's top listened songs tracking
    - `user_status` - Custom showcase sections for profiles
    - `song_plays` - Detailed play tracking for analytics

  2. Security
    - Enable RLS on all new tables
    - Privacy-respecting policies for public/private profiles
    - Follow system with request approval for private accounts

  3. Functions
    - Follow/unfollow management
    - Profile statistics with privacy controls
    - Admin analytics dashboard
    - User search with privacy respect
    - Automatic play tracking and statistics updates

  4. Performance
    - Comprehensive indexing for all query patterns
    - Optimized for social features and analytics
*/
...
BEGIN
  ...
  DROP FUNCTION IF EXISTS send_follow_request(uuid);
  DROP FUNCTION IF EXISTS respond_to_follow_request(uuid, boolean);
  DROP FUNCTION IF EXISTS unfollow_user(uuid);
  DROP FUNCTION IF EXISTS get_user_profile_with_stats(uuid);
  DROP FUNCTION IF EXISTS get_admin_statistics();
  DROP FUNCTION IF EXISTS update_user_status(text, text, uuid);
